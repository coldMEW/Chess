...
namespace Chess
{
    // Central game-state class. It owns the board array, move application, turn order, and timers.
    public class GameControl
    {
        ...
        {
            // Reset all shared state so each new game starts clean.
            if (single == true)
            {
                SinglePlayer = true;
            }
            ...
            sideToMove = BoardSetup.StartingSide;
            Moves = 0;
            KingInCheck = false;
            gameEnded = false;
        ...
        {
            // Stops the clocks, shows the result, and prevents any more moves from being played.
            if (gameEnded == false && winningSide != -1)
            {
                ...
                GameWindow.ShowGameOverControls();
                GameWindow.UpdateTurnDisplay();
                gameEnded = true;
            ...
            else if (gameEnded == false && winningSide == -1) { return false; }
            return true;
        ...
        {
            // The bot becomes more king-focused as pieces disappear and the game gets longer.
            int weighting = 0;
            if(Moves > 30)
            {
                weighting += 3;
        ...
        {
            // Applies one real move to the board, including captures, castling, promotion, sounds, and turn changes.
            if (move.MoveTo == -1)
            {
                if (RuleBook.KingInCheck == true)
                {
                    EndGame(OpposingSide());
                    return;
                }
                EndGame(0);
                return;
            ...
            if (Board[move.MoveFrom].PieceOnSquare != 0)
            {
            ...
            {
                GameWindow.ActiveGameWindow?.BeginComputerTurn();
            }
        }
        ...
        {
            // Used by the bot search to explore a move without committing it permanently.
            ChangeSideToMove();
        ...
        {
            // Restores the board after a temporary search move.
            ChangeSideToMove();
        ...
        {
            if (gameEnded || GameWindow.TimeControl <= 0)
            {
                BlackTimer.Stop();
                WhiteTimer.Stop();
                return;
