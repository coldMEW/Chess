...
namespace Chess
{
    // Generates legal moves and enforces checks, pins, castling, and promotion rules.
    class RuleBook
    {
        ...
        {
            // Precomputes how far each square can move in every direction.
        ...
        {
            // This project always promotes pawns to a queen for simplicity.
        ...
        {
            // Pinned pieces cannot move freely, and checked kings can only be saved by allowed responses.
        ...
        {
            // Builds the attacked-square list from the opponent side's point of view.
        ...
        {
            // Main move-generation pass for the current side to move.
        ...
        {
            // Scans outward from the king to find friendly pieces pinned to it.
            ...
            for (int directionIndex = 0; directionIndex < 8; directionIndex++)
            {
                ...
                for (int i = 0; i < DistanceFromEdges[KingSquare][directionIndex]; i++)
                {
                    ...
                    if (pieceOnTargetSquare != 0 && !Piece.IsColour(pieceOnTargetSquare, SideToGenerateFor))
                    {
                        ...
                        {
                            if((Piece.Type(pieceOnTargetSquare) == Piece.Rook || Piece.Type(pieceOnTargetSquare) == Piece.Queen) && directionIndex < 4)
                            {
                                ...
                                if (Piece.Type(pieceToCheckPinFor) == Piece.Pawn && directionIndex < 2)
                                {
                                    if (Piece.Colour(pieceToCheckPinFor) == Piece.White)
                                    {
                                        ...
                                        if ((locationToCheckPinFor / 8 == 6) && (GameControl.Board[locationToCheckPinFor - 16].PieceOnSquare == 0) && EnemyAttacks.Contains(KingSquare) == false)
                                        {
